...
import { Route, Save, Download, Check, Copy, TriangleAlert, ExternalLink } from 'lucide-react';
...
function Campo({ label, sufijo, valor, onChange, ancho = '' }) {
  return (
...
function Selector({ label, valor, onChange, opciones, ancho = '' }) {
  return (
    <label className={`flex flex-col gap-1 ${ancho}`}>
      <span className="text-xs font-semibold text-navy-500">{label}</span>
...
export default function DisenoViaView({ perfil, projects, onGuardarEnProyecto, onAbrirProyecto }) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl">
      ...
      {/* ---------------------------------------------------- el proyecto */}
      <div className="bg-navy-50 border border-navy-200 rounded-xl p-3 mb-5">
          ...
          {(form.ejes || []).map((eje, i) => {
            ...
            return (
              <div key={eje.tipo} className="flex items-end gap-2 flex-wrap">
                ...
                <Campo label="Cantidad" valor={eje.cantidad} onChange={(v) => setEje(i, 'cantidad', v)} ancho="w-28" />
                <Campo label="Peso por eje" sufijo="ton" valor={eje.peso} onChange={(v) => setEje(i, 'peso', v)} ancho="w-36" />
      ...
      >
        <div className="flex flex-wrap gap-3">
          <Campo label="Tránsito promedio diario (TPD)" valor={form.tpd} onChange={set('tpd')} ancho="w-52" />
          <Campo label="Tasa de crecimiento" sufijo="%" valor={form.tasaPct} onChange={set('tasaPct')} ancho="w-40" />
          <Campo label="Periodo de diseño" sufijo="años" valor={form.periodo} onChange={set('periodo')} ancho="w-40" />
          <Campo label="Factor direccional" sufijo="%" valor={form.direccionalPct} onChange={set('direccionalPct')} ancho="w-40" />
          <Campo label="Vehículos comerciales" sufijo="%" valor={form.comercialesPct} onChange={set('comercialesPct')} ancho="w-44" />
        </div>
      </Panel>

      {/* ------------------------------------------ 3. lluvia y drenaje */}
      <Panel
        titulo="3 · Pluviometría y drenaje"
        pie={<div className="grid grid-cols-2 gap-2">
      ...
      >
        ...
        {/* Los pesos reparten el 100% entre las estaciones: si no suman eso, el
            promedio ponderado no significa lo que dice. */}
        ...
        {r.sumaPesos > 0 && !r.pesosCuadran && (
          <p className="flex items-start gap-1.5 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-2 mb-3">
      ...
      {/* -------------------------------------------------- 4. estructura */}
      <Panel titulo="4 · Materiales y subrasante">
        ...
        <div className="flex flex-wrap gap-3">
          <Selector
            ...
            ancho="w-56"
          />
          <Campo label="Error estándar (So)" valor={form.errorEstandar} onChange={set('errorEstandar')} ancho="w-40" />
          <Campo label="Serviciabilidad inicial" valor={form.servInicial} onChange={set('servInicial')} ancho="w-44" />
          <Campo label="Serviciabilidad final" valor={form.servFinal} onChange={set('servFinal')} ancho="w-44" />
        </div>
      </Panel>

      {/* -------------------------------------------------- 5. resultados */}
      <Panel titulo="5 · Módulos y números estructurales">
